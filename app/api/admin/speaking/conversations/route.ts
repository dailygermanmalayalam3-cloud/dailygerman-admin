...
import { getSpeakingConversations, mutateSpeakingConversation, deleteSpeakingConversation } from "@/lib/db/content";
...
export async function GET(req: Request) {
  try {
    ...
    const items = await getSpeakingConversations(topicId);
...
export async function DELETE(req: Request) {
  try {
    ...
    await deleteSpeakingConversation(id);
    await revalidateLearnerPaths(["/", "/speaking", "/a1", "/a2", "/b1", "/b2"]);
