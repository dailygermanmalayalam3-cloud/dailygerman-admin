...
import { getListeningQuestions, mutateListeningQuestion, deleteListeningQuestion } from "@/lib/db/content";
...
export async function GET(req: Request) {
  try {
    ...
    const items = await getListeningQuestions(topicId);
...
export async function POST(req: Request) {
  try {
    ...
    const item = await mutateListeningQuestion(body);
...
export async function DELETE(req: Request) {
  try {
    ...
    await deleteListeningQuestion(id);
    await revalidateLearnerPaths(["/", "/listening", "/a1", "/a2", "/b1", "/b2"]);
