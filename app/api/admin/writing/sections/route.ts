...
import { getWritingSections, mutateWritingSection, deleteWritingSection } from "@/lib/db/content";
...
export async function GET(req: Request) {
  try {
    ...
    const items = await getWritingSections(topicId);
...
export async function POST(req: Request) {
  try {
    ...
    const item = await mutateWritingSection(body);
