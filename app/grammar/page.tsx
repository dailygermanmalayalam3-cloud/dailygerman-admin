...
export default function AdminGrammarPage() {
  ...
  const handleEdit = (topic: GrammarTopic) => {
    setFormData({
      ...
      explanation_malayalam: topic.explanation_malayalam,
    ...
    try {
      ...
      if (res.ok && data.item) {
        if (editingId) {
          ...
          setStatusMsg({ type: "success", text: "Grammar topic updated!" });
        } else {
          setItems((prev) => [...prev, data.item]);
          setStatusMsg({ type: "success", text: "New grammar topic created!" });
        }
        resetForm();
      } else {
    ...
    <div className="space-y-8">
      <div className="border-b-2 border-black pb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        >
          ...
          <span>{showForm ? "Cancel" : "Add Grammar Topic"}</span>
        </button>
      </div>
      ...
      {showForm && (
        ...
        >
          <h2 className="text-lg font-black uppercase text-black border-b border-black pb-2">
            {editingId ? "Edit Grammar Topic" : "Add New Grammar Topic"}
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider mb-1">Topic Title</label>
              <input
                type="text"
                required
                placeholder="e.g. Das Perfekt mit haben/sein"
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider mb-1">Short Description (English)</label>
            <input
              type="text"
              required
              placeholder="e.g. Forming conversational past tense in German"
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider mb-1">Malayalam Explanation (വിവരണം)</label>
            <textarea
              rows={3}
              required
              placeholder="മലയാളത്തിലുള്ള വ്യാകരണ വിശദീകരണം..."
              value={formData.explanation_malayalam}
              onChange={(e) => setFormData({ ...formData, explanation_malayalam: e.target.value })}
              className="w-full px-3 py-2 border-2 border-black text-sm font-malayalam"
            />
          </div>

          <div>
            <label className="block text-xs font-bold uppercase tracking-wider mb-1">Rule Details & Tables (Content)</label>
            <textarea
              rows={4}
              required
              placeholder="Rules, structures, conjugations..."
      ...
      <div className="space-y-4">
        {loading ? (
          <p className="text-xs text-neutral-500">Loading topics...</p>
        ) : items.length === 0 ? (
          <div className="text-center py-12 border border-neutral-200 bg-neutral-50 p-6">
            <p className="text-neutral-600 text-sm">No grammar topics found.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {items.map((topic) => (
              ...
              >
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="text-base font-black text-black">{topic.title}</h3>
                  </div>
                  <p className="text-xs text-neutral-600 mb-2">{topic.short_description}</p>
                  <p className="text-xs text-amber-950 font-malayalam bg-[#fffbeb] p-2 border-l-2 border-[#facc15]">
